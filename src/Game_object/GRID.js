...
var GRID = new (function () {
    ...
    this.init = function (map) {
        ...
        LATE.AI_WORKER.postMessage("init_map_data", { dim: this.dim, terrian: this.heightmap, material: this.materialmap });
    ...
    this.set_object = function (ob, x, y) {
        if (ob instanceof LATE.Flyable_Unit)
            return;

        x = x || ob.x;
        y = y || ob.y;
        var add = (ob instanceof LATE.Solider_unit) ? 1 : 3;
        ...
        if (!sizeproperty || (sizeproperty.h == 1 && sizeproperty.w == 1)) {
            ...
            if (ob instanceof LATE.Construction_unit || ob instanceof LATE.Tree)
    ...
    this.get_grid_move = function (goal, count_point) {
        ...
        if (goal instanceof LATE.Game_unit || goal instanceof LATE.Map_object || goal instanceof LATE.Grouph_mine)
    ...
    this.get_grid_move_async = function (grouph, goal, farest) {
        ...
        if (goal instanceof Array) {
            ...
            var list_goal = [goal];
            var ob_goal = this.objectmap[goal.x * dim + goal.y].find(e => e instanceof LATE.Construction_unit);
            if (ob_goal)
    ...
    this.setfogmap = function (ob, x, y) {
        if (ob instanceof LATE.Game_unit && ob.team != -1 && ob.property.property.sight) {
            x = x || ob.x;
            y = y || ob.y;
            ...
            if (ob.team == USER_CONTROLER.team_controler.team)
                LATE.FOG_GRAPGICH.set && LATE.FOG_GRAPGICH.set(ob, x, y);
        }
    }
    ...
    this.setmovefogmap = function (ob, bx, by, nx, ny) {
        ...
        if (Math.round(bx) != Math.round(nx) || Math.round(by) != Math.round(ny)) {
            if (ob instanceof LATE.Game_unit && ob.team != -1 && ob.property.property.sight) {
                ...
                if (ob.team == USER_CONTROLER.team_controler.team)
                    LATE.FOG_GRAPGICH.move && LATE.FOG_GRAPGICH.move(ob, bx, by, nx, ny);
            }
        }
    }
    ...
    this.unsetfogmap = function (ob, x, y) {
        if (ob instanceof LATE.Game_unit && ob.team != -1 && ob.property.property.sight) {
            ...
            if (ob.team == USER_CONTROLER.team_controler.team)
                LATE.FOG_GRAPGICH.unset && LATE.FOG_GRAPGICH.unset(ob, x, y);
        }
    }
    ...
    this.check_availble_user_fogmap = function (ob, team) {
        ...
        if (ob instanceof LATE.Game_unit) {
            if (ob.team == userteam)
                return true;
            else
                return (this.fogmap[userteam][Math.round(ob.x) * this.dim + Math.round(ob.y)] > 0);
        } else if (ob instanceof LATE.Game_weapon || ob instanceof LATE.Game_Effect || ob instanceof LATE.Tree) {
            return (this.fogmap[userteam][Math.round(ob.x) * this.dim + Math.round(ob.y)] > 0);
        } 
    }
