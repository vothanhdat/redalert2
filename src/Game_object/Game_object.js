...
import { particlecontainer, stage } from "../Game_Container.js";
import { GRID } from "./GRID.js";
import { SPEED, display_height, display_width, map, screen_x, screen_y } from "../core/state.js";
...
var GAME_OBJECT = new (function () {
    ...
    this.main_loop = function (time) {
        ...
        LATE.WEAPON.static_process(time);
        ...
        LATE.SmokeEffect.process();
        LATE.SmokeEffect2.process();
    ...
    this.add_instance = function (object) {
        if (object instanceof LATE.Game_unit || object instanceof LATE.Map_object || object instanceof LATE.Map_scrap)
            GAME_OBJECT.listobject.push(object);
        else if (object instanceof LATE.Game_weapon)
            GAME_OBJECT.listattack.push(object);
        else if (object instanceof LATE.Game_Effect)
    ...
    this.add_effect = function (x, y, z, property, is_sort, notsound) {
        if (property.class) {
            ...
            return this.add_instance(new LATE.Effect(x, y, z, property, is_sort, notsound));
        }
    }


    setInterval(function () {
        var TMP = sort_unique(GAME_OBJECT.listobject.filter(e => e instanceof LATE.Movealbe_unit).map(e => e.group))
...
function calcfar2_full(ob1, ob2) {
    ...
    if (ob1 instanceof LATE.Construction_unit) {
        dx -= ob1.size.w * 0.5;
        dy -= ob1.size.h * 0.5;
    }
    if (ob2 instanceof LATE.Construction_unit) {
        dx -= ob2.size.w * 0.5;
        dy -= ob2.size.h * 0.5;
    }
    if (dx < 0)
...
Object.assign(LATE, { GAME_OBJECT, calcfar, calcfar2, convert2codinate, convert2screen, convert2screenwithoutsrcpos, get_frame_idx, sort_unique });

export { GAME_OBJECT, Game_object, calcfar, calcfar2, calcfar2_full, calcfar_full, check_available_screen, checkcircle, convert2codinate, convert2screen, convert2screenwithoutsrcpos, get_frame_idx, get_random, normal_angel, null_func, sort_unique };
