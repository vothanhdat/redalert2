...
import { healthcontainer, stage } from "../../Game_Container.js";
import { CONSTRUCTION_TYPE } from "../Construction/Construction_Unit_Type.js";
import { GRID } from "../GRID.js";
import { Game_unit } from "../Game_Unit.js";
import { GAME_OBJECT, calcfar, calcfar2, check_available_screen, convert2screen, get_frame_idx, get_random, normal_angel } from "../Game_object.js";
import { Movealbe_unit } from "./MoviableUnit.js";
import { SOLIDER_TYPE } from "./Solider_Unit/Solider_Unit_Type.js";
import { IMAGE_PROCESS } from "../../Image_process.js";
import { GRAVITY, SPEED } from "../../core/state.js";
...
var PLANE_UNIT = {
    ...
    process_color_gl_done: function (textures, listresource) {
        ...
        LATE.on_texture_load_done("plane");
        PLANE_UNIT = null;
    },
}
...
class Plane_unit_failout extends Game_unit {
    ...
    process(time) {
        ...
        if (this.wait_smooke > 3) {
            this.wait_smooke = 0;
            GAME_OBJECT.add_instance(new LATE.SmokeEffect2(
            ...
            GAME_OBJECT.add_instance(new LATE.SmokeEffect2_supersort(
...
class Parachutist extends Flyable_Unit {
    ...
    init() {
        ...
        GAME_OBJECT.add_instance(new LATE.Parachutist_Sprite(this));
    }
    ...
    process(time) {
        ...
        if ((GRID.heightmap[Math.round(this.x) * GRID.dim + Math.round(this.y)] >= this.z)) {
            ...
            var x = Math.round(this.x),
                y = Math.round(this.y);
            GAME_OBJECT.add_instance(new LATE.Weapon(this, { x, y }, LATE.ATTACK_TYPE.attack_bomb_explorer));
        }
    }
...
PLANE_UNIT_TYPE.beag = {
    ...
    class: class extends Plane_unit {
                ...
                case "attack":
                    if (this.bomb_count > 0) {
                        ...
                        if (this.attackload > attack_delta
                            && LATE.ATTACK_TYPE.attack_bomb_beag.check_can_drop_bomb(this, this.enemy, 2.0)) {
                            //this.attacking = true;
                            this.bomb_count--;
                            this.attackload = 0;
                            this.attack();
        ...
        attack() {
            GAME_OBJECT.add_instance(new LATE.Weapon(this, this.enemy, LATE.ATTACK_TYPE.attack_bomb_beag));
        }
        find_home() {
...
Object.assign(LATE, { ADDITIONAL_TYPE, Flyable_Unit, PLANE_UNIT_TYPE, Parachutist });

export { ADDITIONAL_TYPE, Flyable_Unit, PLANE_UNIT, PLANE_UNIT_TYPE, Parachutist, Plane_unit, Plane_unit_failout, Rocker, V3Rocket, __beag__map__state__ };
