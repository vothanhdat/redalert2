...
import { GAME_OBJECT, calcfar, sort_unique } from "../../Game_object.js";
import { MOVEABLE_UNIT, Movealbe_unit } from "../MoviableUnit.js";
import { Vehicle_unit } from "./Vihicle_Unit.js";
import { STATE, STATE_PLUS } from "../../../lib/JavaScript_helper.js";
        ...
        command(command) {
            switch (command.com) {
                case "set":
                    if (!command.ob)
                        return;
                    if (command.ob instanceof LATE.Heap_mine) {
                        this.workpoint = command.ob.mine;
                        this.force_workpoint = true;
                        this.changetostateplus(STATE_PLUS.MOVE2WORKPOINT);
                    } else if (command.ob instanceof LATE.Mine) {
...
VEHICLE_TYPE.veh_v3rocket = {
    ...
    class: class extends Vehicle_unit {
        ...
        fire() {
            var newinstance = LATE.PLANE_UNIT_TYPE.rocket.get_object(this.x, this.y, this.z + 5, this.team, this.enemy);
...
Object.assign(LATE, { VEHICLE_TYPE });
