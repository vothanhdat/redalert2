...
import { firecontainer, graphics2, particlecontainer, variouscontainer } from "../../Game_Container.js";
import { GRID } from "../GRID.js";
import { GAME_OBJECT, check_available_screen, convert2screen, convert2screenwithoutsrcpos, normal_angel } from "../Game_object.js";
import { Flyable_Unit, Plane_unit } from "../MoviableUnit/Flyable_Unit.js";
import { Movealbe_unit } from "../MoviableUnit/MoviableUnit.js";
import { Game_weapon, WEAPON, Weapon, createline, linetexture_rgb, updateline } from "./Weapon.js";
import { GRAVITY, SPEED, screen_height, screen_width, screen_x, screen_y } from "../../core/state.js";
import { STATE, myAtan, myCos, mySin } from "../../lib/JavaScript_helper.js";
import { Queue } from "../../lib/priorityqueue.js";
...
ATTACK_TYPE.attack_prims = {
    ...
    init (object) {
        ...
        GAME_OBJECT.add_instance(new LATE.Light_Effect(
...
ATTACK_TYPE.attack_tesla = {
    ...
    init (object) {
        object.idx = 0;
        GAME_OBJECT.add_instance(new LATE.Light_Effect_Autoscale(
            object.distination.x,
            object.distination.y,
            object.distination.z,
    ...
    draw (object, static_object) {
        var case_draw = 0;
        if (object.pos1.x > 0 && object.pos1.y > 0 && object.pos1.x < screen_width && object.pos1.y < screen_height)
            case_draw = 1;
        else if (object.pos2.x > 0 && object.pos2.y > 0 && object.pos2.x < screen_width && object.pos2.y < screen_height)
            case_draw = 2;
...
ATTACK_TYPE.attack_rpg = {
        ...
        */
        ...
        if (!object._delete_q) {
            ...
            if (Math.abs(DX) < 0.25 && Math.abs(DY) < 0.25 && Math.abs(DZ) < 2) {
                ...
                GAME_OBJECT.add_instance(new LATE.Effect(object.x + 0.6 * Math.random() - 0.3, object.y + 0.6 * Math.random() - 0.3, object.z, LATE.EFFECT_TYPE.bang2));
                return 1;
            }
        } else {
    ...
    draw (object, static_object) {
        ...
        if (check_available_screen(object.pos1, 40, 40) && GRID.check_availble_user_fogmap(object)) {
            ...
            var srcx = screen_x * 60;
            var srcy = screen_y * 30;
    ...
    process (object, static_ob, time) {
        ...
        if (!object._delete_q) {
            ...
            if ((GRID.heightmap[Math.round(object.x) * GRID.dim + Math.round(object.y)] >= object.z)) {
                ...
                object._delete_q = true;
                GAME_OBJECT.add_instance(new LATE.Effect(object.x + 0.6 * Math.random() - 0.3, object.y + 0.6 * Math.random() - 0.3, object.z, LATE.EFFECT_TYPE.exploit3));
                return 1;
            }
        } else {
            if (object.his.length() > 0)
...
ATTACK_TYPE.attack_cannon = {
    ...
    on_delete (object, static_object) {
        ...
        GAME_OBJECT.add_effect(
            ...
            object.z,
            LATE.EFFECT_TYPE[static_object.atteff]
        );
    },
...
ATTACK_TYPE.attack_ivan_explorer = {
    ...
    static_init () { },
    init (object, static_object) {
        GAME_OBJECT.add_instance(new LATE.Effect(object.x, object.y, object.z, LATE.EFFECT_TYPE.exploit_ivan));
        object.team = -1.5;
    },
    process (object, static_ob, time) {
        if (object.timeline > 15 && !object.isdone) {
...
ATTACK_TYPE.attack_bomb_explorer = {
    ...
    static_init () { },
    init (object, static_object) {
        GAME_OBJECT.add_effect(object.x, object.y, object.z, LATE.EFFECT_TYPE.exploit5);
        object.team = -1.5;
...
Object.assign(LATE, { ATTACK_TYPE });

export { ATTACK_TYPE, _attack_rpg_max_his_, _attack_rpg_step_, _attack_rpg_step_1 };
