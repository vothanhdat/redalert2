...
import { mainstage } from "./Game_Container";
...
export function GameMap(mapname: string) {
    this.texture = [];
    this.terian = [];
    this.material = [];
    this.mapscr2pos = [];
    ...
    var initmapgraphich = async function (resources, main_texture, textture_offset) {
        ...
        var map_renderer = new PIXI.RenderTexture(GLOBAL.renderer, Math.floor(t) * 60, Math.floor(t) * 30);
        ...
        setTimeout(async () => {
            ...
            var minimap_renderer = new PIXI.RenderTexture(GLOBAL.renderer, 242, 242);
    ...
    var loadMapDataAsync = async function (name) {
        ...
        GLOBAL.max_x = this.maxx / 2;
        GLOBAL.max_y = this.maxy / 2;
