...
// This overwrites the native ES2021 String.prototype.replaceAll with a different
// signature (a plain-string search plus an ignoreCase flag, no RegExp support).
// Nothing in the codebase calls it. The cast keeps the clobber visible rather than
// letting it masquerade as the standard method.
(String.prototype as any).replaceAll = function (search, replace, ignoreCase) {
    if (ignoreCase) {
        var result = [];
...
export { GlowFilter, GlowFilter2, InverseAlpha, KEYBOARD, Lighter4x, NoiseFilter, STATE, STATE_PLUS, TeamColorFilter, ____coeff_1, ____coeff_2, check_point_inside, myAtan, myCos, mySin, random, reverseArray };
